...
namespace Sample.CRUD.Repository.EntityFramework.GenericRepository
{
    public class BaseRepository : IBaseRepository
    {
        private readonly SampleCRUD_Employee_DbContext _context;
        public BaseRepository(SampleCRUD_Employee_DbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IEnumerable<T>> GetAllAsync<T>(params Expression<Func<T, object>>[] includeProperties) where T : class, IAudit
        {
            ...
            if (includeProperties != null)
            {
                foreach (var property in includeProperties)
                    await set.Include(property).LoadAsync();
            }
